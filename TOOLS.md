...
| **git** | CLI | Version control, handoff motion | `C:\Program Files\Git` | 2026-09-09 | Free | ⚠️ Reach it as `git -C "<path>"`; `cd <dir> && git …` is blocked by the Bash sandbox. ⚠️ **Also (S16): `git commit -m` is blocked in Bash — use PowerShell.** Bash also rejects `for` loops with `${var}` expansion and multi-command `&&`/`;` chains |
