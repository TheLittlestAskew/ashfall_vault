 ...
 * (falling back to the registry "name" only if a fetch fails), so the registry
 * just needs IDs — correct PC names appear automatically on the first good run.
 *
 * AUTH (optional):
 *   By default this runs ANONYMOUS — it only sees sheets shared as Public.
 *   If DDB_COBALT is present in the vault .env, it authenticates AS YOU: the
 *   Cobalt session cookie is exchanged for a short-lived Bearer token sent on
 *   every fetch, which unlocks "Campaign Only" sheets for campaigns you're in.
 *   (It does NOT unlock other people's truly-Private sheets.) The Ashfall party
 *   sheets are all currently Campaign-Only, so a token is needed to fetch them.
 *
 ...
 *   it IS your live login session). It's account-wide, so the SAME value you use
 *   in any other vault works here. Lasts weeks; refresh when fetches 403 again.
 *
