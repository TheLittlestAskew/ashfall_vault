---
session: 3
title: Mr. Cat, Esquire
date: 2026-02-24
roll-archive-date: 2026-02-24
campaign: Ashfall Britannia
party-level: 5
tags: [session-notes, ashfall, cliffhanger]
---

# Session 03 — Mr. Cat, Esquire

> ⚠ **ENDS MID-COMBAT** — the campaign's first cliffhanger. Two recordings unified (121 + 13 min; ~18-min gap reconstructed from the roll archive).

## Metadata

| | |
|---|---|
| **Session Number** | 03 |
| **Session Date** | 02/24/2026 (played 8:16–11:00 PM ET; recording started ~12 min late) |
| **Start Location** | [[The Old Hospital]] — second floor (nurses' dormitory) |
| **End Location** | Same — **PAUSED MID-COMBAT** (1 wounded [[Beast Thrall]] standing) |
| **Party Present** | All 7 PCs + **Mr. Cat** (Steel Defender, named this session) |
| **Total Rolls** | **≈184 total** = 163 DDB-archive (**[[Valerian Hellebore]]'s PC rolls MISSING — sync gap ⚑**; first-ever DM-account rolls: 2, both pre-recording) + **~21 transcript-only DM/enemy rolls** (tallied in §6 Logs) |
| **Party Level** | 5 — XP posted to Discord post-session (amount TBD ⚑) |
| **Spelling Checked** | Yes — `022426_Spell_Check_Log.md` |

## POV Journal
> See [[Vega Bloodroot Journal]] — entry "The hospital, still. Night."

## Summary

### Narrative Summary

Direct continuation of S02: up to the hospital's **second floor** (nurses' dormitory — meteor-era bloodstains, dust thick enough to read). Tracks: beasts, plus **the Beast Master's fine boot-prints again** — he toured this building too. Scraping, metal thuds, faint **sniffing** in the dark.

[[Deanna Smith-Wesson]]'s recon collapsed in style — a Bardic-rerolled Stealth that got *worse* (8→6) and a literal fall **through** a door — triggering a session-long multi-wave brawl: feral thralls and wall-jumping beast thralls, including ambushers from behind. Mid-fight the DM announced a **KILL CONTEST** (per-character tally, prize for the most this session — reward wording garbled ⚑).

Highlights: the Steel Defender was christened **MR. CAT, ESQUIRE** (tuxedo paint + monocle marking planned); [[Samothy Smith-Wesson]] delivered the play of the night — **hookshot fired through a feral thrall, anchored in the wall, riding the cable to smash it apart** (next-session inspiration awarded) — plus a 29-to-hit pistol headshot (*"It's high noon"*). [[Valerian Hellebore]], still grieving and *"fucking glowing,"* opened the scoring with a Starry Wisp head-shot and ran Moonbeam control all night; his nat-20 Dex save became a Cirque du Soleil corpse-dodge. [[Zelda Z Whipper]] wall-walked with the hookshot bracers and kept [[Barrett Grimmskar]] alive through a 21-damage chest wound (2× Healing Word + Lunar Vitality). [[Flux]] built **two homebrew items mid-session** (the Hookshot Arm Bracers card; Vega's **Double-Bladed Scimitar**, unlocking her Sneak Attack — debut hit: 18). Vega's full mid-session turn fell in the recording gap and is archive-reconstructed.

At ~11 PM: **paused mid-combat**, one badly wounded beast thrall standing. Kill standings: Samothy 2 · Flux 2 · Valerian 1+ · Barrett 1 · Vega/Zelda/Deanna 0.

### Locations Visited

| Location | Description | Notable |
|---|---|---|
| [[The Old Hospital]] 2F — nurses' dormitory | Old blood, cobwebs, gurneys, barrels | Beast Master boot-prints; multi-wave ambush; UNRESOLVED |

## Threads
- ⚔️ **FINISH THE FIGHT** — cliffhanger; S04 should open mid-initiative.
- 🔄 Supplies / Bobby's book / nirnroot — pharmacy, admin, basement all unreached.
- 🏆 Kill contest — prize TBD ⚑; standings logged.
- 👞 Beast Master's boots on 2F — he was in the hospital too. OPEN.
- ⚑ Pre-recording group **Wisdom save vs unknown** (Samothy 4, Flux nat-1 → both failed) — ask DM.
- Carryovers unchanged: spy · Beast Tamer · missing children · Varan · rendezvous clock.

## Scenes

1. *(pre-recording)* Session start + mystery group Wisdom save ⚑
2. 2F recon — tracks, sniffing
3. Deanna's door-fall → initiative ×2 (VTT auto-roller mutiny)
4. Wave 1 — Valerian's head-shot kill; Zelda's goat-jump Thunderwave
5. Kill contest announced; Mr. Cat summoned & named
6. Wave 2 from behind — Samothy prone, AC-22 cheese ("as you level up, so do the monsters")
7. Hookshot showcase — wall-walk, Catapult barrel, the Spider-Man cable-kill
8. Barrett's chest wound + "Kill steal!"; scimitar delivery + Sneak Attack debut
9. *(recording gap — archive)* Vega's turn + enemy turns
10. Final on-record round → **PAUSED** at 11 PM

### Themes

Grief metabolized as radiance · toys become doctrine (mobility meta) · competition as bonding · Mr. Cat crosses from equipment to family · the boots again — quiet dread under the comedy.

## Character Activity
**Party:** single group, hospital 2F, all 7 + Mr. Cat — combat unresolved.

**NPCs:** Mr. Cat, Esquire (construct companion — named, beloved, deflect-tank MVP) · the unseen Beast Master (boot-prints) · thralls (~6 slain, 1 wounded standing).

**Relationships:** Jill's mid-session crafting earned Vega's "You're a beautiful human and I love you" · Taylorito/Taylorita coined for the two Taylors · Samothy's stunt-economy with the DM matures (cool points → inspiration).

## Artifacts
> [[Loot Tracker S01-S10]] § S03. Headlines: **Hookshot Arm Bracers formalized** (Samothy's homebrew card — +1 AC *requires attunement*, bonus action, 4 charges @ lvl 5, tri-hook); **Vega's Double-Bladed Scimitar** (Jill's homebrew, 2d4+4 finesse → Sneak Attack); no new loot (rooms unsearched — combat unresolved).

## Logs
**Encounter:** multi-wave ferals + beast thralls, hospital 2F — **UNRESOLVED**; ~6 slain; Barrett took 21 (healed 9+4, then 8); Samothy self-inflicted 6 (cable-kill); Valerian's exact HP unknown (sync gap).

**Initiative (re-roll, archive-verified):** Barrett 24 (adv 12,20) → Zelda 23 → Flux 22 → Deanna 11 → Samothy 10 → Vega 10 → Valerian (unsynced). Table order: Barrett → Zelda → Flux → enemy → Deanna → Valerian → Samothy+Mr. Cat → Vega → enemies.

> Full annotated roll log in the `.docx`; stats in [[Roll Statistics S01-S10]] § S03. Notable: Samothy's Str-check "dirty 20" cable-kill + 29 [19] headshot; Deanna's worse-after-reroll Stealth; Zelda's Lucky reroll (nat 1 → still missed); Barrett's Risk Die rolling a 1; the recording-gap Vega turn (DBS 23 → 9+3+3, dagger 8, miss 12); archive-only Flux Fire Bolt ⚑.

#### Transcript-only DM / enemy rolls (~21 — not in the DDB archive)

> Approximate: the fight pauses mid-combat, spans a recording gap, and the VTT auto-roller + diarization blobs swallow some enemy rolls. Enemy initiative was VTT-rolled (values lost in the auto-roller chaos).

| Roll | Target | Value | Context |
|---|---|---|---|
| Attack | Deanna | 17 | miss (+1-AC bracers) |
| Attack | Vega | 20 | hit |
| Damage | Vega | 13 | bludgeoning → halved (Totem resist) |
| Attack ×2 | Zelda | nat 20, 11 | 20 hits |
| Damage | Zelda | 13 | claw |
| Attack ×2 | Samothy | 24, 26 | net miss — Mr. Cat Deflect + AC 22; Sam knocked prone |
| Attack ×2 | Barrett | nat 20, 23 | both hit |
| Damage | Barrett | 8 | 15 total, halved |
| Attack | Valerian | 22 | hit; forced Str save (passed) |
| Damage | Valerian | 20 | claw |
| Attack | Barrett | 19 | hit — the 21-dmg chest wound |
| Damage | Barrett | 21 | chest wound |
| Con save ×2 | thralls | both pass | vs Zelda's Thunderwave (values unvoiced) |
| Con save | thrall | 13 | vs Valerian's Moonbeam — fails, 17 radiant |
| Con save | thrall | nat 20 | vs Moonbeam (Pt 2) — passes, half (13) |
| Dex save | thrall | 23 | vs Samothy's Catapult barrel — passes, half (8) |
| Dex save | thrall | 14 | vs Deanna's Perforating Shot — passes, half |

> **Tally:** ~10 enemy attacks · 6 saves · 5 damage = **~21 transcript-only** (enemy initiative additionally VTT-rolled, values lost). With the 163 archive rolls → **≈184 total.** The beasts rolled **three nat 20s** (attacks on Zelda and Barrett both landed; a clutch Moonbeam save) — while Mr. Cat's Deflect and the +1-AC bracers turned away several other hits.

## Quotes & Language
> [[Quote Board S01-S10]] § S03 · [[Profanity Ledger S01-S10]] § S03 (Chase three-peats, 20).

**Final title:** *Mr. Cat, Esquire* (Character — Taylor). Alternates: It's High Noon (Quote) · Kill Count (Event) · The Second Floor (Location) · To Be Continued (Thematic).

## Archivist Notes

- **S04 must open mid-combat** — verify continuity against the 030426 recording.
- Valerian sync gap ⚑ · pre-recording Wisdom save ⚑ · kill-contest reward garbled ⚑ · "Eldritch Maul" DDB label ⚑ · XP amount in Discord ⚑ · Mr.-Cat-husband attribution ⚑ · "Kira" garble.
- **Naming:** the DM and table repeatedly call the beast thralls "the beast tamer" (e.g. the kill-contest line, "the beast tamer's turn") — table shorthand for the beasts, **not** the unseen [[The Beast Tamer]] (Beast Master), who appears here only as boot-prints on 2F. **"Orphe"** (Vega's aside, 00:31) = Orphie Levistus, Taylor's PC in another campaign — not Ashfall canon.
- Vega's rebuild materializing piecemeal (Totem-resistance talk, Savage Attacker on sheet, finesse project done) — formal sheet state unconfirmed ⚑.
- Deanna's two-session damage drought is now a table bit.
